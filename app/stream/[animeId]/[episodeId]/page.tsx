...
export default function WatchPage({ params }: WatchPageProps) {
    ...
    useEffect(() => {
        if (episodes) {
            const matchingEpisode = episodes.find(
                (episode) => episode.number === parseInt(params.episodeId)
            );
    ...
    const recommendationsArray: (MediaFragment | null)[] =
        info?.Media?.recommendations?.nodes
            ...
            <div className="px-2 lg:px-12 pt-6">
                ...
                <div className="pt-6">
                    ...
                    <div className="grid grid-cols-4 mt-4">
                        <div className="flex flex-col col-span-3 lg:flex-row items-center lg:items-start text-center lg:text-start">
                                    ...
                                    <div>
                                        <h1 className="text-2xl font-bold">
                                            {info?.Media?.title?.english ??
                                                info?.Media?.title?.userPreferred}
                                        </h1>
                                        <p className="text-sm font-bold text-muted-foreground">
                                            {info?.Media?.title?.romaji} •{" "}
                                            {info?.Media?.title?.native}
                                        </p>
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                <div className="px-6 lg:px-0 pt-2 h-fit w-[95%]">
                                                    <p className="text-sm font-medium text-muted-foreground mt-2">
                                                        ...
                                                        {info?.Media?.status
                                                            ? info?.Media?.status?.charAt(0) +
                                                            info?.Media?.status?.slice(1).toLowerCase()
                                                            : "-"}
                                                    </p>
                                                    <p className="text-sm font-medium text-muted-foreground mt-2">
                                                        <span className="text-primary font-semibold">
                                                        ...
                                                        {info?.Media?.genres?.map((name, index) => (
                                                            <>
                                                                <Link
